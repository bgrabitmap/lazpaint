...
uses
  ...
  {$IFDEF FPC_FULLVERSION>=49525}
  LazUTF8,
  {$ENDIF}
  BGRABitmapTypes, InterfaceBase, BGRAPen, BGRAGrayscaleMask;
...
begin
  ...
  p := 1;
  inc(p, {$IFDEF FPC_FULLVERSION>=49525}}LazUTF8.{$ENDIF}UTF8CharacterLength(@ATextUTF8[p])); //UTF8 chars may be more than 1 byte long
  ...
  begin
    ...
    totalWidth := TextSizeNoUpdateFont(copy(ATextUTF8,1,p+
      {$IFDEF FPC_FULLVERSION>=49525}}LazUTF8.{$ENDIF}UTF8CharacterLength(@ATextUTF8[p])-1)).cx; //copy whole last UTF8 char
    ...
    inc(p, LazUTF8.UTF8CharacterLength(@ATextUTF8[p]));
  end;
  ARemainsUTF8 := '';
end;
